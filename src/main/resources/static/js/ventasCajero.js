document.addEventListener('DOMContentLoaded', function () {
    ...
    // --- ID del cajero logueado (se carga desde /api/auth/perfil) ---
    let cajeroUserId = null;
    ...
    async function loadVentas(page = 0) {
        ...
        try {
            ...
            // Filtrar automáticamente por el cajero logueado
            const vendedorParam = cajeroUserId ? `&vendedorId=${cajeroUserId}` : '';
    ...
    async function loadProductosParaSelect() {
        try {
            ...
            console.log("Productos actualizados en Ventas Cajero:", todosLosProductos.length);
        } catch (error) {
    ...
    // Cargar el ID del cajero logueado para filtrar solo sus ventas
    async function loadcajeroUserId() {
        try {
            const response = await fetch('/api/auth/perfil');
            if (!response.ok) return;
            const perfil = await response.json();
            cajeroUserId = perfil.idUsuario;
            console.log('cajero userId cargado:', cajeroUserId);
        } catch (e) {
            console.error('Error al cargar perfil del cajero:', e);
        }
    }

    // Inicialización secuencial: primero cargar userId, luego ventas
    (async function init() {
        await loadcajeroUserId();
    ...
    document.addEventListener('productosActualizados', function () {
        console.log('VentasCajero.js: Detectada actualización de productos. Recargando lista...');
        loadProductosParaSelect();
    });
